...
export const onboardingIdentitySchema = z.object({
  displayName: z
    ...
    .min(
      1,
      "Informe como você gostaria de ser chamado.",
    )
    .max(
      60,
...
export const onboardingRepresentationSchema =
  z.enum([
    "ngo",
    "company",
  ]);
...
export const onboardingParticipationSchema = z
  .object({
    roles: z
      ...
      .max(
        4,
        "Não é possível selecionar mais de quatro perfis pessoais.",
      ),

    representations: z
      .array(
        onboardingRepresentationSchema,
      )
      .max(
        2,
    ...
    (data, context) => {
      if (
        ...
        context.addIssue({
          ...
          path: [],
          message:
            "Escolha pelo menos uma forma de participar.",
        });
      }

      const uniqueRoles =
        new Set<CollaborationRole>(
          data.roles,
        );
      ...
      if (
        ...
        context.addIssue({
          ...
          path: ["roles"],
          message:
            "O mesmo perfil não pode ser selecionado mais de uma vez.",
        });
      }

      const uniqueRepresentations =
        new Set<OnboardingRepresentation>(
          data.representations,
        );

      if (
        uniqueRepresentations.size !==
...
export type OnboardingParticipationInput =
  z.infer<
