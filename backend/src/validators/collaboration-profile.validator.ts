...
const volunteerAvailabilityDetailsSchema = z.object({
  ...
  if (data.frequency !== "flexible" && (data.days.length === 0 || data.periods.length === 0)) {
    context.addIssue({
      code: "custom",
      path: ["days"],
...
const volunteerProfileDataSchema = z.object({
  interestAreas: z.array(textItemSchema).min(1).max(10),
  availability: z.string().trim().max(240).optional(),
  causes: z.array(textItemSchema).min(1).max(30).superRefine((causes, context) => {
  ...
  opportunityPreference: z.enum(["recurring", "punctual", "both"]).default("both"),
}).strict();
...
export function parseCollaborationProfileData(
  ...
  return profileDataSchemas[role].parse(profileData) as CollaborationProfileData;
}
