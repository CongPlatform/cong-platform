...
import { BadgeInfo, CalendarClock, LoaderCircle, MapPin, Plus, X } from "lucide-react";
...
const OPPORTUNITY_OPTIONS = [
  {
    value: "recurring",
    label: "Recorrentes",
    description: "Participações contínuas em rotinas, equipes ou projetos ativos.",
  },
  {
    value: "punctual",
    label: "Pontuais",
    description: "Ações com começo e fim definidos, como eventos, mutirões e campanhas.",
  },
  {
    value: "both",
    label: "Ambos",
...
function availabilityText(details: AvailabilityDetails): string {
  ...
  const periodText = details.periods.length ? details.periods.join(" e ").toLowerCase() : "";
  const frequencyText = details.frequency ? FREQUENCY_LABELS[details.frequency] : "";
...
function availabilityPieces(details: AvailabilityDetails) {
  ...
  return {
    ...
    periods: details.periods.length ? details.periods.join(" e ") : "Períodos não definidos",
    frequency: details.frequency ? FREQUENCY_LABELS[details.frequency] : "Frequência não definida",
  };
}
    ...
    <div className={styles.selectedList}>
      {values.map((value) => (
        <span className={styles.chip} key={value}>
          ...
          <button type="button" onClick={() => onRemove(value)} aria-label={`Remover ${value}`}>
            <X aria-hidden="true" />
          </button>
        </span>
      ))}
...
export default function VolunteerProfileForm(props: Props) {
  ...
  const update = (patch: Partial<VolunteerProfileFormData>) => {
    props.onChange({
      ...
      availabilityDetails: patch.availabilityDetails ?? props.availabilityDetails,
      opportunityPreference: patch.opportunityPreference ?? props.opportunityPreference,
    });
  };
  ...
  const submit = async (event: SubmitEvent<HTMLFormElement>) => {
    ...
    const availabilityComplete =
      ...
      (details.days.length > 0 && details.periods.length > 0 && Boolean(details.frequency));
    ...
    <>
      <form className={styles.form} onSubmit={submit} noValidate>
        <header className={styles.header}>
          ...
          <h2>Voluntário</h2>
          <p>Vamos usar suas preferências para encontrar oportunidades melhores.</p>
        </header>

        <section className={styles.section}>
          <div className={styles.compactHeading}>
            <h3>Causas que gostaria de apoiar <span aria-hidden="true">*</span></h3>
          </div>
          <SelectedItems
            values={props.causes}
            ...
            onRemove={(value) => {
              ...
              update({
                causes: parsed.subtopic
                  ...
                  : props.causes.filter(
                      (item) => parseCauseSelection(item).parent !== parsed.parent,
                    ),
              });
            }}
          />
          <button type="button" className={styles.addButton} onClick={() => setCauseModalOpen(true)}>
        ...
        <section className={styles.section}>
          <div className={styles.compactHeading}>
            <h3>Como gostaria de ajudar <span aria-hidden="true">*</span></h3>
          </div>
          <SelectedItems
            values={props.interestAreas}
            onRemove={(value) =>
              update({ interestAreas: props.interestAreas.filter((item) => item !== value) })
            }
          />
          <button type="button" className={styles.addButton} onClick={() => setActivityModalOpen(true)}>
            <Plus aria-hidden="true" />
            Escolher atividades
          </button>
        </section>
        ...
        <section className={styles.section}>
          <div className={styles.compactHeading}>
            <h3>Onde pretende atuar <span aria-hidden="true">*</span></h3>
          </div>
          ...
          <div className={styles.locationGrid}>
            ...
            <div className={styles.inlineField}>
              ...
              <select
                ...
                onChange={(event) => {
                  update({ location: { ...location, city: event.target.value } });
                  setError("");
                }}
              >
                <option value="">
                  {citiesLoading ? "Carregando cidades..." : location.state ? "Selecione" : "Escolha a UF primeiro"}
                </option>
          ...
          {citiesError && <button type="button" className={styles.textAction} onClick={() => void loadCities(location.state ?? "")}>{citiesError}</button>}
          ...
          <label className={styles.checkboxLine}>
            <input
              ...
              onChange={(event) =>
                update({ location: { ...location, remote: event.target.checked } })
              }
            />
            ...
            {OPPORTUNITY_OPTIONS.map((option) => (
              <button
                ...
                className={`${styles.choiceCard} ${
                  props.opportunityPreference === option.value ? styles.choiceCardSelected : ""
                }`}
        ...
        <section className={styles.section}>
          <div className={styles.compactHeading}>
            <h3>Disponibilidade <span aria-hidden="true">*</span></h3>
          </div>
          {availabilityText(details) ? (
            <div className={styles.availabilityCard}>
              <div className={styles.availabilityHeader}>
                ...
                <button type="button" onClick={() => setAvailabilityModalOpen(true)}>Alterar</button>
              </div>

              <div className={styles.availabilityMeta}>
                <div>
                  <span className={styles.summaryLabel}><CalendarClock aria-hidden="true" /> Dias</span>
                  <strong>{availabilitySummary.days}</strong>
                </div>
                <div>
                  <span className={styles.summaryLabel}><BadgeInfo aria-hidden="true" /> Períodos</span>
                  <strong>{availabilitySummary.periods}</strong>
                </div>
                <div>
                  <span className={styles.summaryLabel}><MapPin aria-hidden="true" /> Frequência</span>
                  <strong>{availabilitySummary.frequency}</strong>
                </div>
              </div>
            </div>
          ) : (
            <button type="button" className={styles.addButton} onClick={() => setAvailabilityModalOpen(true)}>
              <Plus aria-hidden="true" />
        ...
        {props.onOpenParticipationChoices && (
          <button type="button" className={styles.inlineLink} onClick={props.onOpenParticipationChoices}>
